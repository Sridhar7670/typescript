...
//     sound = "Engine roar";
//     fly() {
//         console.log("Flying like a plane!");
//     }
// }

// function makeItFly(creature: Bird) {
//     creature.fly();
// }

// let donald: Bird = new Duck(); // Works because Duck has the shape of Bird
// makeItFly(donald);

// let boeing: Bird = new Plane(); // Works because Plane has the shape of Bird
// makeItFly(boeing);
...
// structural typing is a system where type compatibility is based on an object's shape or structure (the properties it holds) rather than its explicit declaration. This is often called "duck typing" from the phrase: "If it walks like a duck and it quacks like a duck, then it must be a duck."

// If an object has the same properties and types as an interface requires, TypeScript considers it compatible, even if it wasn't explicitly created from that interface.


// Example:

// Define the "contract" with an interface. We expect an object with a name (string) and a speak method.
